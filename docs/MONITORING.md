...
| Memory | total/available/used, cache/slab/anonymous/dirty, Swap, zswap, and selected vmstat values | `/proc/meminfo`, `/proc/vmstat` | 1 s / 5 s |
| PSI | CPU/memory/I/O `some`/`full` avg/total | `/proc/pressure/*` | 1 s / 2 s |
| Block devices | bytes/s, IOPS, busy, in-flight, queue size, estimated read/write latency | `/proc/diskstats`, block sysfs | 1 s / 3 s |
| Network interfaces | byte/packet/error/drop rates, operstate, MTU, carrier/duplex, and speed when available | `/proc/net/dev`, `/sys/class/net` | 1 s / 3 s |
...
| Processes | `(pid,starttime)`, PPID, state, CPU, RSS/VSZ, thread count, priority/nice/CPU, UID | `/proc/<pid>/stat`, `status` | 1 s / 5 s |
| CPUFreq | policy/CPU set, current/min/max, driver, governor, boost/EPP | cpufreq sysfs | 1 s / 5 s |
| hwmon | temperature, fan RPM, voltage, current, power, energy, thresholds/alarms/faults; known invalid sentinel values are filtered | `/sys/class/hwmon` | 1 s / 5 s |
