...
| Cross-platform runtime | Linux release path remains; source-development macOS arm64 and Windows x86 backends provide CPU identity and utilization, memory composition, processes, disk I/O, mounts, network, sockets (Windows), system, TUI, Snapshot, Agent, and diagnose on tested hosts; the Server 2008 legacy console is exercised through a console harness | XP runtime, Cygwin hang-up handling, Windows/macOS sensors, GPU and workloads, older macOS, performance baselines |
