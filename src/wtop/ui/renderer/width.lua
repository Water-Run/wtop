-- Unicode display-cell measurement used by every UI component.
--
-- This is deliberately independent from the terminal backend.  A backend (or
-- a test) can inject a more complete wcwidth implementation through
-- `width_fn`; the bundled implementation is a conservative Unicode 15-ish
-- fallback suitable for Linux terminals.
local M = {}
...
-- Zero-width marks, format characters, variation selectors and joiners.
-- The ranges are kept sorted so lookup stays cheap on redraw-heavy paths.
local combining = {
...
-- East Asian Wide/Fullwidth and emoji ranges commonly rendered as two cells.
local wide = {
  ...
  {0x3041, 0x33FF}, {0x3400, 0x4DBF}, {0x4E00, 0xA4C6},
  {0xA960, 0xA97C}, {0xAC00, 0xD7A3}, {0xF900, 0xFAFF},
  {0xFE10, 0xFE19}, {0xFE30, 0xFE6B}, {0xFF01, 0xFF60},
  {0xFFE0, 0xFFE6}, {0x16FE0, 0x16FE3}, {0x17000, 0x18D08},
  {0x1AFF0, 0x1AFF3}, {0x1AFF5, 0x1AFFB}, {0x1AFFD, 0x1AFFE},
  {0x1B000, 0x1B122}, {0x1B132, 0x1B132}, {0x1B150, 0x1B152},
  {0x1B155, 0x1B155}, {0x1B164, 0x1B167}, {0x1F004, 0x1F004},
  {0x1F0CF, 0x1F0CF}, {0x1F18E, 0x1F18E}, {0x1F191, 0x1F19A},
  {0x1F200, 0x1F202}, {0x1F210, 0x1F23B}, {0x1F240, 0x1F248},
  {0x1F250, 0x1F251}, {0x1F260, 0x1F265}, {0x1F300, 0x1F64F},
  {0x1F680, 0x1F6FF}, {0x1F700, 0x1F773}, {0x1F780, 0x1F7D8},
  {0x1F7E0, 0x1F7EB}, {0x1F800, 0x1F80B}, {0x1F810, 0x1F847},
...
-- A compact subset of East Asian Ambiguous ranges.  Most Linux terminals
-- render these as one cell; CJK users can opt into two or inject wcwidth.
local ambiguous = {
  ...
  {0x3248, 0x324F}, {0xE000, 0xF8FF}, {0xFE00, 0xFE0F},
  {0xFFFD, 0xFFFD}, {0x1F100, 0x1F10A}, {0x1F110, 0x1F12D},
  {0x1F130, 0x1F169}, {0x1F170, 0x1F18D}, {0x1F18F, 0x1F190},
  {0x1F19B, 0x1F1AC}, {0xE0100, 0xE01EF},
}

local function decode_at(text, index)
  ...
  local length, minimum, cp
  if first >= 0xC2 and first <= 0xDF then
    length, minimum, cp = 2, 0x80, first - 0xC0
  elseif first >= 0xE0 and first <= 0xEF then
    length, minimum, cp = 3, 0x800, first - 0xE0
  elseif first >= 0xF0 and first <= 0xF4 then
    length, minimum, cp = 4, 0x10000, first - 0xF0
  else
    return 0xFFFD, index + 1, false
  end

  if index + length - 1 > #text then
    return nil, index, false, "incomplete"
  end
  ...
  return cp, index + length, true
end

M.decode_at = decode_at

function M.codepoint_width(cp, options)
  options = options or {}
  if options.width_fn then
    local ok, result = pcall(options.width_fn, cp, options)
    if ok and type(result) == "number" and result == result
        and result ~= math.huge and result ~= -math.huge and result >= 0 and result <= 16 then
  ...
  if cp < 0x20 or (cp >= 0x7F and cp < 0xA0) then
    -- Grid.write renders controls as visible, safe replacement cells (except
    -- line breaks, which terminate a write). Measurement must agree or right
    -- alignment can overlap neighbouring content.
    return (cp == 0x0A or cp == 0x0D) and 0 or 1
  end
  if cp == 0x200D or in_ranges(cp, combining) then
    return 0
  end
  if in_ranges(cp, wide) then
    return 2
  end
  if options.ambiguous_is_wide and in_ranges(cp, ambiguous) then
    return 2
  end
  return 1
end
...
local function is_regional(cp)
  return cp >= 0x1F1E6 and cp <= 0x1F1FF
end

-- Iterate terminal grapheme-ish clusters.  Full UAX #29 segmentation is not
-- required for layout, but keeping combining marks, emoji modifiers, flags and
-- ZWJ sequences together prevents the common half-emoji/half-CJK corruption.
function M.graphemes(text, options)
  options = options or {}
  local index = 1
  return function()
    if index > #text then
    ...
    local start = index
    local cp, next_index, valid = decode_at(text, index)
    if not cp then
      -- A trailing invalid byte is rendered as a replacement character.
      cp, next_index = 0xFFFD, index + 1
      valid = false
    end
    index = next_index
    local cluster_width = M.codepoint_width(cp, options)
    local regional = is_regional(cp)
    local regional_count = regional and 1 or 0
    local joined = false
    local emoji_presentation = false

    while index <= #text do
      local next_cp, after = decode_at(text, index)
      if not next_cp then
        break
      end
      local take = false
      if is_extender(next_cp) then
        take = true
        if next_cp == 0x200D then
          joined = true
        elseif next_cp == 0xFE0F or (next_cp >= 0x1F3FB and next_cp <= 0x1F3FF) then
          emoji_presentation = true
        end
      elseif joined then
        take = true
        joined = false
        cluster_width = math.max(cluster_width, M.codepoint_width(next_cp, options))
      elseif regional and regional_count == 1 and is_regional(next_cp) then
        take = true
        regional_count = 2
        cluster_width = 2
      end
    ...
    -- Emoji presentation selector and joiner sequences occupy one glyph cell
    -- group even though several component codepoints may individually be wide.
    if regional or emoji_presentation
        or text:sub(start, index - 1):find("\226\128\141", 1, true) then
...
function M.pad(text, width, align, options)
  ...
  local remaining = math.max(0, width - used)
  if align == "right" then
    return string.rep(" ", remaining) .. clipped
  elseif align == "center" then
    local left = math.floor(remaining / 2)
    return string.rep(" ", left) .. clipped .. string.rep(" ", remaining - left)
  end
  return clipped .. string.rep(" ", remaining)
end
